...
- 设置 `GO_SERVER_OPTIONS_PROVIDER=massive` 并填写独立的 `MASSIVE_OPTIONS_API_KEY` 可启用通用美股期权合约和日线接口。免费档使用 `MASSIVE_OPTIONS_PLAN_NAME=options_basic`、`MASSIVE_OPTIONS_REQUESTS_PER_MINUTE=5`；Developer 会员使用 `MASSIVE_OPTIONS_PLAN_NAME=options_developer`、`MASSIVE_OPTIONS_REQUESTS_PER_MINUTE=0`（本地不限次），支持 4 年历史和 15 分钟延迟行情；期权调用量独立保存在 `options-usage.db`，响应缓存保存在 `options-cache.db`。接口只提供市场数据，不包含交易策略。
